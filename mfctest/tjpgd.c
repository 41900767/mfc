...
{ 
    ...
    //    f_read(dev,buf,num,&rb);//调用FATFS的f_read函数，用于把jpeg文件的数据读取出来
		rb = fread(buf, 1, num, dev);
        return rb;        		//返回读取到的字节数目
	}else {

		fseek(dev, num, SEEK_CUR);
		return num;
	}
...
void CopyRect16(unsigned short*pdst, unsigned short *psrc )
{
...
{ 
	...
	u16 *pdst = g_buf + (rect->top * g_Width + rect->left);
	int i, j;
	if(rect->left < 0 || rect->right > g_Width)
		return 0;
	if(rect->top < 0 || rect->bottom > gHeight)
		return 0;
...
{  
	...
	g_buf = pdst;
	f_jpeg = fopen(filename, "rb");
	res=jd_prepare(jpeg_dev, jpeg_in_func, jpg_buffer, JPEG_WBUF_SIZE, f_jpeg);//执行解码的准备工作，调用TjpgDec模块的jd_prepare函数
	outfun=jpeg_out_func_fill;//默认采用画点的方式显示
	jd_decomp(jpeg_dev, outfun, 0);
	fclose(f_jpeg);
	return 0;
}
